...
class IowaLiquorSalesService(object):
    ...
    def get_item_data_agg_by_month(self):
        df = pd.DataFrame.from_dict(self.query("SELECT itemno, vendor_no, date, invoice_line_no, pack, sale_bottles, sale_dollars, state_bottle_retail, state_bottle_cost WHERE itemno='19068' AND vendor_no='65' ORDER BY date ASC LIMIT 50000")).set_index('invoice_line_no')
        return df
...
if __name__=='__main__':
    ...
    iowa_liquor_sales_service = IowaLiquorSalesService()
    iowa_liquor_sales_service.get_item_data_agg_by_month().to_csv('./tmp/item_sales.csv')